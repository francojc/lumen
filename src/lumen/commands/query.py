"""lumen query — translate natural language to lumen search query syntax."""
...
def _build_command(parsed: dict) -> str:
    """Render a parsed query dict as a ``lumen search …`` command string."""
    parts = ["lumen search"]
...
def query(
    ...
    if run:
        ...
        from lumen.commands.search import search as _search
